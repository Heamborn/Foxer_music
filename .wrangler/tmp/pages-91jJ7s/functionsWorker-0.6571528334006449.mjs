...
// ../.wrangler/tmp/bundle-VY1mcO/checked-fetch.js
...
// ../.wrangler/tmp/bundle-VY1mcO/strip-cf-connecting-ip-header.js
...
var SAFE_RESPONSE_HEADERS = ["content-type", "cache-control", "accept-ranges", "content-length", "content-range", "etag", "last-modified", "expires", "location"];
...
async function proxyApiRequest(url, request) {
  ...
  const isUrlRequest = apiUrl.searchParams.get("type") === "url";
  const upstream = await fetch(apiUrl.toString(), {
    headers: {
      "User-Agent": request.headers.get("User-Agent") ?? "Mozilla/5.0",
      "Accept": "application/json"
    },
    redirect: isUrlRequest ? "manual" : "follow"
    // type=url时不自动跟随重定向
  });
  if (isUrlRequest && upstream.type === "opaqueredirect") {
    const followUpstream = await fetch(apiUrl.toString(), {
      headers: {
        "User-Agent": request.headers.get("User-Agent") ?? "Mozilla/5.0",
        "Accept": "application/json"
      },
    ...
    headers2.set("Access-Control-Expose-Headers", "Location");
    headers2.set("Location", followUpstream.url);
    return new Response(null, {
      status: 302,
      statusText: "Found",
      headers: headers2
...
// ../.wrangler/tmp/pages-91jJ7s/functionsRoutes-0.34099955194338016.mjs
var routes = [
  {
    routePath: "/api/login",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-VY1mcO/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-VY1mcO/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.6571528334006449.mjs.map
